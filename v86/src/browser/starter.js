...
V86.prototype.continue_init = async function(emulator, options)
{
    ...
    var cont = function(index)
    {
        ...
        console.log("Loading file " + (index + 1) + "/" + total + ": " + f.name + " (" + f.url + ")");
    ...
    async function done()
    {
        console.log("V86Starter done() called. fs9p:", !!settings.fs9p, "fs9p_json:", !!settings.fs9p_json);
        if(settings.fs9p && settings.fs9p_json)
        {
            if(!settings.initial_state)
            {
                ...
                if(options.bzimage_initrd_from_filesystem)
                {
                    ...
                    console.log("Found bzimage: " + bzimage_path + " and initrd: " + initrd_path);

                    const [initrd, bzimage] = await Promise.all([
                        (initrd_path && !settings.initrd && !options.disable_initrd) ? settings.fs9p.read_file(initrd_path) : Promise.resolve(null),
                        (bzimage_path && !settings.bzimage) ? settings.fs9p.read_file(bzimage_path) : Promise.resolve(null),
                    ]);
                    if(initrd && !settings.initrd) put_on_settings.call(this, "initrd", new SyncBuffer(initrd.buffer));
                    if(bzimage && !settings.bzimage) put_on_settings.call(this, "bzimage", new SyncBuffer(bzimage.buffer));
                }
            }
            else
            {
...
V86.prototype.get_bzimage_initrd_from_filesystem = function(filesystem)
{
    ...
    for(const f of [].concat(root, boot))
    {
        ...
        const is_bzimage = name === "bzImage" || /^vmlinuz(-.*)?$/i.test(name);
        const is_initrd = name === "initrd" || /^initrd(\.img)?(\.cpio\.gz)?$/i.test(name) || /^initramfs(-.*)?$/i.test(name) || name.endsWith("-initrd");
    ...
    {
        const search_stack = ["/", "/var/guix/profiles/system", "/boot", "/gnu/store"];
        const visited = new Set();
        let limit = 2000;
        while(search_stack.length > 0 && limit-- > 0 && (!initrd_path || !bzimage_path))
        {
            const dir = search_stack.pop();
            if(visited.has(dir)) continue;
            visited.add(dir);

            const entries = filesystem.read_dir(dir);
            if(!entries) continue;

            const sorted_entries = Array.from(entries).sort((a, b) => {
                const aKey = /linux-libre|initrd|system|boot/i.test(a) ? 1 : 0;
                const bKey = /linux-libre|initrd|system|boot/i.test(b) ? 1 : 0;
                return aKey - bKey;
            });

            for(const item of sorted_entries)
            {
                const full = (dir === "/" ? "" : dir) + "/" + item;
                const p = filesystem.SearchPath(full);
                if(p.id !== -1)
                {
                    if(filesystem.IsDirectory(p.id))
                    {
                        search_stack.push(full);
                    }
                    else
                    {
                        const inode = filesystem.GetInode(p.id);
                        const is_reg = inode && (inode.mode & 0xF000) === 0x8000;
                        if(is_reg)
                        {
                            const is_bzimage = item === "bzImage" || /^vmlinuz(-.*)?$/i.test(item);
                            const is_initrd = item === "initrd" || /^initrd(\.img)?(\.cpio\.gz)?$/i.test(item) || /^initramfs(-.*)?$/i.test(item);

                            if(is_bzimage && !bzimage_path)
                            {
                                bzimage_path = full;
                                console.log("Found bzimage path: " + bzimage_path);
                            }
                            if(is_initrd && !initrd_path)
                            {
                                initrd_path = full;
                                console.log("Found initrd path: " + initrd_path);
                            }
                        }
                    }
                }
    ...
    if(!initrd_path || !bzimage_path)
    {
        console.log("Failed to find bzimage or initrd in filesystem.");
    }
