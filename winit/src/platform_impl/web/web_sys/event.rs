...
use crate::keyboard::{
    Key, KeyLocation, ModifiersState, NamedKey, PhysicalKey,
};
...
impl MouseDelta {
    pub fn init(window: &web_sys::Window, event: &PointerEvent) -> Self {
        match super::engine(window) {
            ...
            Some(Engine::Gecko) if has_coalesced_events_support(event) => {
                Self::Gecko {
    ...
    pub fn delta(&mut self, event: &MouseEvent) -> Position {
        match self {
            MouseDelta::Chromium => {
                PhysicalPosition::new(event.movement_x(), event.movement_y())
                    .into()
            }
            MouseDelta::Gecko {
                old_position,
            ...
            MouseDelta::Other => {
                LogicalPosition::new(event.movement_x(), event.movement_y())
                    .into()
            }
        }
    }
